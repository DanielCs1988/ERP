...
def print_table(table, title_list, order_by=1):
    """
    ...
    for row in qsort_table(table, order_by, key=lambda x: int(x) if isinstance(x, int) or x.isnumeric() else str(x).lower()):
        count = 0
